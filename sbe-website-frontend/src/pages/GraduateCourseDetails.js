...
function GradCourseDetails(isAuthenticated) {
  ...
  useEffect(() => {
    axios.get(`http://localhost:8000/api/graduatecourse/${params.id}`).then((res) => {
      setCourse(res.data);
  ...
  useEffect(() => {
    axios.get("http://localhost:8000/api/uploadmaterialsgrad/").then((res) => {
  ...
  const handleSubmit = (e, type) => {
    ...
    if (type == "mat") {
      axios
        .post("http://localhost:8000/api/uploadmaterialsgrad/", fileData)
        .then((res) => {
          console.log(res);
        })
        .catch((e) => {
    ...
    if (type == "mat-link" || type == "grades") {
      ...
      axios
        .put(`http://localhost:8000/api/graduatecourse/${params.id}`, formData)
        .then((res) => {
          console.log(res);
        })
        .catch((error) => {
  ...
  const deleteCourse = (id) => {
    axios
      .delete(`http://localhost:8000/api/graduatecourse/${id}`)
      .then((res) => {
                  ...
                  <div className="col-4">
                    {isCoordinator || isAdmin ? (
                      <Link
                        ...
                        style={{  color: "#ffff" }}
                        to={`/assigngradcourse/${course.id}/${course.name}/`}
                        onClick={()=>goToAssign()}
                      >
                  ...
                  <div className=" col-8 text-end ">
                    ...
                    {isCoordinator || isAdmin ? (
                      <Link
                        ...
                        to={`/editgraduatecourse/${course.id}/${course.name}/${course.total_grade}/${course.instructions}/${course.staff_id}/${course.year}`}
                      >
                    ...
                    <div className="col-12">
                      <button className="button btn cards ">
                        <Link
                          ...
                          className="ani fs-5"
                          to={`/graduatecourse-history/${course.id}`}
                        >
                          Show Course History
                        </Link>
                      </button>
...
export default GradCourseDetails;
